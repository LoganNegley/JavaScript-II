...
let donationSum = runners.reduce(function(accumulator, item) {
  return accumulator + item.donation;
}, 0);
donationSum = ticketPriceTotal;
...
// Problem 1
const highDonations = [];
let prizeWinners = runners.filter(item => {
  return item.donation > 200;
});
highDonations.push(prizeWinners);
